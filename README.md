...
1. **Run** — the exact script that generates that analytical stage.
2. **Inspect** — the compact CSV outputs that should be checked first.
3. **See** — an example figure generated by the validated worked example.
4. **Interpret** — an example of scientifically cautious language for describing the result.

The example values below come from the validated repository dataset. When the pipeline is applied to a new project, the **workflow and interpretation rules stay the same but the numerical results must be regenerated**.
...
The complete one-command workflow was rerun successfully on the worked example on **20 August 2026**, beginning with input validation and proceeding through Processing Steps 01–07 and Analyses 20–28 without a fatal error.

The final run reconfirmed:

| Check | Validated result |
|---|---:|
| Source annotation rows | 55,096 |
| Raw-export frames | 525 |
| Point-sampled frames | 522 |
| Frames exactly at nominal 100-point target | 430 |
| Frames with depth/temperature metadata | 209 |
| Final point-table rows | 51,955 |
| Final VME-table rows | 3,141 |
| Main community-model eligible frames | 430 |
| Main community groups | 73 |
| Final analysis reached | Analysis 28 — VME occurrence |

> [!NOTE]
> Informational package-build notices and expected diagnostic warnings may appear during execution. A validated run is defined by successful completion of every required stage plus review of the stage-specific QC/eligibility/convergence outputs, not by the complete absence of console warnings.
...
- [`07_processing_pipeline_manifest.csv`](Analyses/00_Quality_Control/07_processing_pipeline_manifest.csv)
- [`20_frame_qc_summary.csv`](Analyses/00_Quality_Control/20_frame_qc_summary.csv)
- [`20_point_class_counts.csv`](Analyses/00_Quality_Control/20_point_class_counts.csv)
- [`20_community_unit_resolution_summary.csv`](Analyses/00_Quality_Control/20_community_unit_resolution_summary.csv)
- [`20_frame_metadata_completeness.csv`](Analyses/00_Quality_Control/20_frame_metadata_completeness.csv)

### Example interpretation

> The processing workflow preserved all 55,096 source rows, separated random-point and targeted-VME sampling designs without loss, and explicitly identified deviations from the nominal point-sampling target and structured environmental-metadata missingness. These QC flags are retained rather than corrected silently.
...
*Figure 1. Random-point count QC across point-sampled frames; deviations from the nominal target remain visible rather than being forced to 100.*

![Frame composition by dive](Figures/Fig_02_Frame_Composition_By_Dive.png)

*Figure 2. Worked-example frame composition by dive, showing the relative contribution of annotation classes.*
...
- [`21_alpha_diversity_summary.csv`](Analyses/02_Alpha_Diversity/21_alpha_diversity_summary.csv)
- [`21_alpha_model_eligibility.csv`](Analyses/02_Alpha_Diversity/21_alpha_model_eligibility.csv)

### Example interpretation

> Local frames contained a median of five resolved biological community units. Rarefaction reduced this to an expected median richness of approximately 3.77 units at a standardised effort of 10 assigned biotic points. Nine point-sampled frames contained no resolved biota and were retained rather than removed.
...
*Figure 3. Example observed common-ID richness versus depth output.*

![Shannon diversity versus depth](Figures/Fig_04_Shannon_Depth.png)

*Figure 4. Example Shannon diversity versus depth output.*

![Shannon diversity by substrate](Figures/Fig_05_Shannon_Substrate.png)

*Figure 5. Example Shannon diversity by frame substrate class.*
...
- [`22_nmds_model_summary.csv`](Analyses/03_Community_Composition/NMDS/22_nmds_model_summary.csv)
- [`22_nmds_dimension_comparison.csv`](Analyses/03_Community_Composition/NMDS/22_nmds_dimension_comparison.csv)
- [`22_nmds_environmental_fit_frame_level.csv`](Analyses/03_Community_Composition/NMDS/22_nmds_environmental_fit_frame_level.csv)
- [`22_nmds_environmental_fit_dive_level.csv`](Analyses/03_Community_Composition/NMDS/22_nmds_environmental_fit_dive_level.csv)

### Example output: ordination dimensionality

| NMDS dimensions | Stress | Relative reduction from 2-D |
|---:|---:|---:|
| 2 | 0.2122 | 0% |
| 3 | 0.1575 | 25.8% |
...
> The 2-D NMDS is useful as a visual summary but represents the Bray–Curtis rank structure only weakly. A 3-D solution reduces stress by approximately 25.8% and is the preferred low-dimensional representation for interpretation. Temperature and substrate show the clearest exploratory associations with the ordination, while latitude and longitude do not.
...
*Figure 6. Two-dimensional NMDS visualisation with geographic context; interpret alongside the reported 2-D stress and 3-D diagnostic.*

![NMDS by substrate](Figures/Fig_07_NMDS_Substrate.png)

*Figure 7. Two-dimensional NMDS coloured/grouped by substrate class.*

![NMDS by relief](Figures/Fig_08_NMDS_Relief.png)

*Figure 8. Two-dimensional NMDS by relief class; PERMDISP and replication diagnostics are required for inference.*
...
- [`23_permanova_results.csv`](Analyses/04_PERMANOVA/23_permanova_results.csv)
- [`23_permdisp_results.csv`](Analyses/04_PERMANOVA/23_permdisp_results.csv)
- [`23_permanova_sample_sizes.csv`](Analyses/04_PERMANOVA/23_permanova_sample_sizes.csv)
- [`23_permanova_interpretation_summary.csv`](Analyses/04_PERMANOVA/23_permanova_interpretation_summary.csv)

### Example interpretation

> Temperature and substrate produce the strongest raw PERMANOVA signals, but neither standalone term crosses the predeclared global BH threshold of q < 0.05. Strong substrate PERMDISP means any substrate-related PERMANOVA pattern cannot be attributed solely to differences in multivariate centroids. Relief shows dispersion differences without corresponding centroid evidence.
...
- [`24_dbrda_model_diagnostics.csv`](Analyses/05_CAP_dbRDA/24_dbrda_model_diagnostics.csv)
- [`24_dbrda_overall_test.csv`](Analyses/05_CAP_dbRDA/24_dbrda_overall_test.csv)
- [`24_dbrda_term_tests.csv`](Analyses/05_CAP_dbRDA/24_dbrda_term_tests.csv)
- [`24_dbrda_vif.csv`](Analyses/05_CAP_dbRDA/24_dbrda_vif.csv)

### Example interpretation

> The complete-case environmental dbRDA is supported overall and explains an adjusted 35.3% of community dissimilarity within the 141-frame, six-dive subset. Temperature is the only individual term supported after BH correction within this four-term constrained model. The result should therefore be described as strong evidence within the complete-case subset, not as 35% of variation explained across the entire survey.
...
*Figure 9. Constrained dbRDA of the complete-case environmental subset.*

![Dive-level geographic dbRDA](Figures/Fig_10_CAP_dbRDA_Dive_Latitude.png)

*Figure 10. Dive-level geographic dbRDA; the adjusted R² is approximately zero in the worked example.*
...
- [`25_environmental_analysis_notes.txt`](Analyses/06_Environmental_Associations/25_environmental_analysis_notes.txt)
- [`25_environmental_sample_sizes.csv`](Analyses/06_Environmental_Associations/25_environmental_sample_sizes.csv)
- [`25_environmental_regressions.csv`](Analyses/06_Environmental_Associations/25_environmental_regressions.csv)
- [`25_environmental_gams.csv`](Analyses/06_Environmental_Associations/25_environmental_gams.csv)

### Example interpretation

> After respecting the dive hierarchy, warmer within-dive observations are associated with lower observed richness, rarefied richness, Shannon diversity and inverse Simpson diversity. Agreement between blocked Spearman tests and hierarchical GAMs suggests that this temperature-diversity pattern is not dependent on one model form. Model R² values include dive-level variance and must not be attributed to temperature alone.
...
- [`26_factor_test_eligibility.csv`](Analyses/07_Univariate_Tests/26_factor_test_eligibility.csv)
- [`26_global_hierarchical_factor_tests.csv`](Analyses/07_Univariate_Tests/26_global_hierarchical_factor_tests.csv)
- [`26_pairwise_test_eligibility.csv`](Analyses/07_Univariate_Tests/26_pairwise_test_eligibility.csv)
- [`26_pairwise_hierarchical_factor_tests.csv`](Analyses/07_Univariate_Tests/26_pairwise_hierarchical_factor_tests.csv)

### Example interpretation

> Once dive is controlled for, substrate class does not explain detectable differences in frame-level richness, diversity, evenness or biotic point percentage. Relief cannot be tested inferentially for these responses because only one eligible dive contains within-dive relief variation. The pipeline reports that lack of replication rather than producing a pseudoreplicated p-value.
...
- [`27_focal_taxa_selected.csv`](Analyses/08_Taxon_Responses/27_focal_taxa_selected.csv)
- [`27_continuous_model_eligibility.csv`](Analyses/08_Taxon_Responses/27_continuous_model_eligibility.csv)
- [`27_continuous_taxon_models.csv`](Analyses/08_Taxon_Responses/27_continuous_taxon_models.csv)
- [`27_model_fit_diagnostics.csv`](Analyses/08_Taxon_Responses/27_model_fit_diagnostics.csv)
- [`27_models_excluded_numerical_instability.csv`](Analyses/08_Taxon_Responses/27_models_excluded_numerical_instability.csv)

### Example interpretation

> The community-wide temperature response is not a uniform decline across taxa. Brittle-star cover decreases with temperature, whereas burrowing-clam cover increases. This indicates reorganisation of relative dominance among focal biological groups. The validated rerun contains no focal-taxon model excluded for numerical instability.
...
*Figure 11. Prevalence of the focal community units selected for hierarchical taxon-response models.*
...
- [`28_vme_presence_by_dive.csv`](Analyses/09_VME/28_vme_presence_by_dive.csv)
- [`28_vme_type_prevalence.csv`](Analyses/09_VME/28_vme_type_prevalence.csv)
- [`28_continuous_vme_models.csv`](Analyses/09_VME/28_continuous_vme_models.csv)
- [`28_factor_vme_models.csv`](Analyses/09_VME/28_factor_vme_models.csv)
- [`28_model_fit_diagnostics.csv`](Analyses/09_VME/28_model_fit_diagnostics.csv)

### Example interpretation

> VME annotations occur in approximately 71% of screened point frames. Within the available metadata subset, VME occurrence increases with depth and decreases with temperature, while relief is also associated with VME occurrence. These models treat unannotated screened frames as absences; that interpretation is valid only when VME screening was consistently applied across the analysed frame universe.
...
*Figure 12. Frame-level VME occurrence/prevalence by dive. Annotation totals are not treated as random-point abundance.*

## Complete worked-example figure gallery

The same figures are placed beside their relevant analyses above. This compact gallery provides a single visual index to all validated worked-example outputs.

<table>
<tr>
<td width="50%"><b>Fig. 01 — Point Count QC</b><br><img src="Figures/Fig_01_Point_Count_QC.png" width="100%"></td>
<td width="50%"><b>Fig. 02 — Frame Composition by Dive</b><br><img src="Figures/Fig_02_Frame_Composition_By_Dive.png" width="100%"></td>
</tr>
<tr>
<td><b>Fig. 03 — Richness vs Depth</b><br><img src="Figures/Fig_03_Richness_Depth.png" width="100%"></td>
<td><b>Fig. 04 — Shannon vs Depth</b><br><img src="Figures/Fig_04_Shannon_Depth.png" width="100%"></td>
</tr>
<tr>
<td><b>Fig. 05 — Shannon by Substrate</b><br><img src="Figures/Fig_05_Shannon_Substrate.png" width="100%"></td>
<td><b>Fig. 06 — NMDS Latitude</b><br><img src="Figures/Fig_06_NMDS_Latitude.png" width="100%"></td>
</tr>
<tr>
<td><b>Fig. 07 — NMDS Substrate</b><br><img src="Figures/Fig_07_NMDS_Substrate.png" width="100%"></td>
<td><b>Fig. 08 — NMDS Relief</b><br><img src="Figures/Fig_08_NMDS_Relief.png" width="100%"></td>
</tr>
<tr>
<td><b>Fig. 09 — Environmental dbRDA</b><br><img src="Figures/Fig_09_CAP_dbRDA_Environment.png" width="100%"></td>
<td><b>Fig. 10 — Dive Geographic dbRDA</b><br><img src="Figures/Fig_10_CAP_dbRDA_Dive_Latitude.png" width="100%"></td>
</tr>
<tr>
<td><b>Fig. 11 — Focal-Taxon Prevalence</b><br><img src="Figures/Fig_11_Focal_Taxon_Prevalence.png" width="100%"></td>
<td><b>Fig. 12 — VME Occurrence by Dive</b><br><img src="Figures/Fig_12_VME_Occurrence_By_Dive.png" width="100%"></td>
</tr>
</table>
...
| Final clean full-pipeline rerun from canonical inputs | **Completed successfully — 20 Aug 2026** |
| Software lockfile / exact environment capture | Planned |
| Versioned release tag | Pending final documentation/environment audit |

---

## Immediate Next Development Step

The analytical modules have been individually executed, reviewed and hardened on the worked example, and the complete `./Scripts/run_full_pipeline.sh .` workflow has now been rerun successfully from the canonical prerequisite inputs.

The remaining release-stage tasks are:

1. record the R/package environment used for the validated release;
2. perform a final README/output/version-control audit;
3. optionally archive a checksum or compact validation manifest for the release outputs; and
4. create a versioned Git tag for the validated pipeline release.
